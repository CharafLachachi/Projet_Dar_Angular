...
export class AuthenticationService {
    ...
    login(email: string, password: string) {
        return this.http.post<any>(`https://localhost:8080/DAR_PROJECT/SignIn`, { email, password })
            .pipe(map(token => {
    ...
    register(user : any){
        ...
        return this.http.post<any>(`https://localhost:8080/DAR_PROJECT/SignUp`, user)
            .pipe(map(resp => {
                return resp;
            }));
    }
